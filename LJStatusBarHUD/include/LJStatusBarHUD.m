...
static const NSTimeInterval LJStatusBarHUDDuration = 0.5;

@implementation LJStatusBarHUD : NSObject
...
+ (void)showMessage:(NSString *)msg image:(NSString *)image
{
    ...
    // 动画
    [UIView animateWithDuration:LJStatusBarHUDDuration animations:^{
        _window.transform = CGAffineTransformIdentity;
    } completion:^(BOOL finished) {
        [UIView animateWithDuration:LJStatusBarHUDDuration delay:1.0 options:UIViewAnimationOptionCurveLinear animations:^{
            _window.transform = CGAffineTransformMakeTranslation(0, -_window.height);
        } completion:^(BOOL finished) {
            _window.hidden = YES;
        }];
